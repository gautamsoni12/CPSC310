...
export class Rooms {
                ...
                if (file != null) {
                    jsZip.loadAsync(file, {base64: true}).then(function (zip: any) {
                        ...
                        Promise.all(promiseArray).then(function (response: any) {
                            ...
                            fulfill(x.comleteRoom(x.buildings, x.rooms));
    ...
    comleteRoom(buildingsArray: Array<any>, roomsArray: Array<any>): Promise<Array<any>> {
        let x = this;

        return new Promise(function (resolve, reject) {
            var latlonPrimiseArray: Array<any> = [];
            try {
                buildingsArray = x.buildings;
                roomsArray = x.rooms;
                    ...
                    for (let room of roomsArray) {
                        if (building.building_shortname === room.rooms_shortname) {
                            ...
                            latlonPrimiseArray.push(new Promise(function (fulfill, reject) {
                                    x.getlatLon(building.building_address, room).then(function (value: any) {
                                        fulfill(value);
                                        //console.log(value);
                ...
                Promise.all(latlonPrimiseArray).then(function (response: any) {
                    x.rooms = x.rooms.filter(function (r_obj) {
    ...
    getlatLon(address: string, room: any): Promise<any> {
        return new Promise(function (resolve, reject) {
        ...
        if (typeof htmlNode === 'object') {
            if (htmlNode.childNodes != null) {
                ...
                for (let child of children) {
                    if (x.tableCheck(child)) {
                        x.getBuildings(child);
                    }
                    x.recurse(child);
                }
            }
